...
## Overview
This project implements a Genetic Algorithm that evolves a compositional picture made of simple geometric primitives (such as circles, lines, triangles, ellipses) to approximate a given target image. Each chromosome represents an entire image composed of 50 layered translucent genes-primitives. The GA mutates and recombines these primitives to reduce the pixel-wise difference to the target image.

## Problem Description
The core problem is to recreate a given 32×32 target image using a limited number of overlapping figures. The GA is used to search for an optimal combination of shape parameters that minimizes the difference to the target.
...
- **Shape Type**: e.g., ellipse, line, triangle or circle.
- **Position**: x and y coordinates on the 32×32 canvas.
- **Size**: dimensions like radius, width/height or list of points.
...
## Goals
- Reconstruct a recognizable 32×32 image using 50 translucent primitives.
